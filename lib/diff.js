...
export const patchDocumentNode = (schema, oldNode, newNode) => {
  ...
  console.log(`==> eq left:${left}, right:${right}`)
...
const matchNodes = (schema, oldChildren, newChildren) => {
  ...
  for (let oldStartIndex = 0; oldStartIndex < oldChildren.length; oldStartIndex++) {
    ...
    console.log('==> matching', newStartIndex !== -1, oldStartIndex, newStartIndex)
    if (newStartIndex !== -1) {
      ...
      for (; oldEndIndex < oldChildren.length && newEndIndex < newChildren.length; oldEndIndex++, newEndIndex++) {
        ...
        const newEndNodeIndex = findMatchNode(newChildren, oldEndNode, newEndIndex)
        if (newEndNodeIndex === -1) {
          break
        }
      }
